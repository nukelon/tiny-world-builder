...
- Keep shadow bias/normalBias/radius tight for voxel-scale geometry. Large `normalBias` or soft radius values make thin roofs, columns, crop stems, fences, and trim detach from their shadows, which reads as light leaking through the model in pixel mode. Shadow-casting FrontSide materials should use `material.shadowSide = THREE.FrontSide` via the shared helper so one-sided roof/trim faces are present in the shadow map.
